...
use grapple_frc_msgs::{
    binmarshal::{BinMarshal, BitView, BitWriter, LengthTaggedVec, VecBitWriter},
    bridge::BridgedCANMessage,
    MessageId,
};

use crate::{
    hal_safe_call, HAL_CANStreamMessage, HAL_CAN_CloseStreamSession, HAL_CAN_OpenStreamSession,
    HAL_CAN_ReadStreamSession, HAL_CAN_SendMessage, HAL_CAN_SEND_PERIOD_NO_REPEAT,
};
    ...
    loop {
        ...
        if read_buf.len() >= 2 {
            ...
            if (read_buf.len() - 2) >= msg_len {
                ...
                let bridged_msg = BridgedCANMessage::read(&mut BitView::new(&read_buf[2..]), ())
                    .ok_or(anyhow::anyhow!("Invalid Message!"))?;
        ...
        let mut stream_messages: [HAL_CANStreamMessage; 1024] = [HAL_CANStreamMessage {
            ..Default::default()
        }; 1024];
        let mut n_read = 0u32;
        let result = hal_safe_call!(HAL_CAN_ReadStreamSession(
            session_handle,
            &mut stream_messages as *mut HAL_CANStreamMessage,
            1024,
            &mut n_read as *mut u32
        ));
        ...
        match result {
            Ok(_) => {
                for msg in &stream_messages[0..n_read as usize] {
                    ...
                    let bridged_msg = BridgedCANMessage {
                        id: message_id,
                        timestamp: msg.timeStamp,
                        data: LengthTaggedVec::new(msg.data[0..msg.dataSize as usize].to_vec()),
                    };
...
fn start_can_bridge() -> anyhow::Result<()> {
    ...
    for stream in server.incoming() {
        ...
        hal_safe_call!(HAL_CAN_OpenStreamSession(
