...
use crate::{
    hal_safe_call, with_err, HAL_CAN_ReceiveMessage, HAL_CAN_SendMessage,
...
pub extern "C" fn lasercan_set_timing_budget(inst: *mut LaserCanDevice, budget: u8) -> c_int {
    unsafe { with_err((*inst).send(LaserCanMessage::SetTimingBudget { budget })) }
}

#[no_mangle]
pub extern "C" fn lasercan_set_roi(inst: *mut LaserCanDevice, roi: LaserCanRoi) -> c_int {
    unsafe { with_err((*inst).send(LaserCanMessage::SetRoi { roi })) }
}

#[no_mangle]
pub extern "C" fn lasercan_set_range(inst: *mut LaserCanDevice, long: bool) -> c_int {
    unsafe { with_err((*inst).send(LaserCanMessage::SetRange { long })) }
}
...
pub extern "system" fn Java_au_grapplerobotics_LaserCan_setTimingBudget<'local>(
    ...
    try_send(
        &mut env,
        lc,
        LaserCanMessage::SetTimingBudget {
            budget: budget as u8,
        },
    )
}
